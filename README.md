...
- **Checks**: the worker re-validates tracked domains on their own cadence (`[tracking]` in `config.ini`) and reports signals: **reputation** (abuse.ch + VirusTotal), **WHOIS/NS changes**, **DNS resolution** (Google DoH), **TLS certificate issuance** (crt.sh, key-less) and **HTTP content** (the matched keyword/brand in the title/body, a login form, a plain 200 and a content hash for change detection).
