<?php
/**
 * Queue a batch VirusTotal lookup on the worker.
 *
 * The free VirusTotal API allows 4 requests/minute and 500/day, so the batch is
 * capped (VT_BATCH_MAX) and domains already cached are skipped unless force=1.
 */
...
// Domains already queued in a pending/running vt_lookup command.
$pendingCommandId = null;
$pendingDomains = [];
foreach ($db->query("SELECT id, payload FROM commands WHERE command = 'vt_lookup' AND status IN ('pending','running') ORDER BY id ASC")->fetchAll() as $c) {
...
if (!$force) {
    ...
    $stmt = $db->prepare("SELECT domain FROM domain_vt WHERE domain IN ($placeholders)");
    $stmt->execute($clean);
...
$db->prepare("INSERT INTO commands (command, payload) VALUES (?, ?)")
   ->execute(['vt_lookup', json_encode(['domains' => $toFetch])]);
...
echo json_encode([
    ...
    'message' => 'Queued for the worker. The free API allows 4/min and 500/day.',
]);
