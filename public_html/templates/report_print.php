...
<div class="rp">
        ...
        <?php foreach ($sections as $sec): $c = $sec['counts']; ?>
            <div class="rp-kw">
                ...
                <table class="rp-table">
                    ...
                    <tbody>
                        <?php foreach ($sec['rows'] as $er): $r = $er['row']; $status = $er['status']; $rep = $er['rep']; $abuse = reportAbusech($r); ?>
                        <tr>
                            ...
                            <td><span class="rp-rep rp-rep-<?= htmlspecialchars($rep['state']) ?>"><?= htmlspecialchars($repSymbol[$rep['state']] ?? '') ?> <?= htmlspecialchars($rep['label']) ?></span><?php if ($rep['detail'] !== ''): ?> <span class="rp-muted">(<?= htmlspecialchars($rep['detail']) ?>)</span><?php endif; ?><?php if (in_array($abuse['state'], ['malicious', 'suspicious', 'clean'], true)): ?> <span class="rp-rep rp-rep-<?= htmlspecialchars($abuse['state']) ?>">AC <?= htmlspecialchars($abuse['label']) ?></span><?php endif; ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <div class="rp-dd">
                ...
                <div class="rp-dd-block">
                    ...
                    <dl>
                        ...
                        <div><dt>Abuse.ch</dt><dd><span class="rp-rep rp-rep-<?= htmlspecialchars($abuse['state']) ?>"><?= htmlspecialchars($repSymbol[$abuse['state']] ?? '') ?> <?= htmlspecialchars($abuse['label']) ?></span><?php if ($abuse['detail'] !== ''): ?> (<?= htmlspecialchars($abuse['detail']) ?>)<?php endif; ?></dd></div>
    ...
    <div class="rp-foot">
        ...
        Sources: WHOIS/RDAP + VirusTotal + abuse.ch (URLhaus/ThreatFox) (DNS, passive DNS, TLS and IP/ASN are not integrated).
    </div>
</div>
