<?php
/**
 * Queue a batch AlienVault OTX lookup on the worker.
 *
 * OTX is queried one domain at a time; the batch is capped (OTX_BATCH_MAX) and
 * domains already cached are skipped unless force=1.
 */
...
// Domains already queued in a pending/running otx_lookup command.
$pendingCommandId = null;
$pendingDomains = [];
foreach ($db->query("SELECT id, payload FROM commands WHERE command = 'otx_lookup' AND status IN ('pending','running') ORDER BY id ASC")->fetchAll() as $c) {
...
if (!$force) {
    ...
    $stmt = $db->prepare("SELECT domain FROM domain_otx WHERE domain IN ($placeholders)");
    $stmt->execute($clean);
...
foreach ($clean as $d) {
    ...
    $toFetch[] = $d;
    if (count($toFetch) >= OTX_BATCH_MAX) {
        break;
    }
}
...
$db->prepare("INSERT INTO commands (command, payload) VALUES (?, ?)")
   ->execute(['otx_lookup', json_encode(['domains' => $toFetch])]);
