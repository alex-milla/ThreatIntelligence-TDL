...
def send_vt_results(host_url: str, api_key: str, entries: list[dict]) -> bool:
    """Send VirusTotal domain reputations to the hosting API."""
    if not entries:
        return True
    url = f"{host_url}/api/v1/vt_results.php"
    headers = {
        "X-API-Key": api_key,
        "Content-Type": "application/json",
    }
    ...
    if r.status_code == 200:
        return True
    print(f"[-] Failed to send VirusTotal results: HTTP {r.status_code} - {r.text}")
    return False


def send_abusech_results(host_url: str, api_key: str, entries: list[dict]) -> bool:
    """Send abuse.ch (URLhaus + ThreatFox) domain validations to the hosting API."""
    if not entries:
        return True
    url = f"{host_url}/api/v1/abusech_results.php"
    headers = {
        "X-API-Key": api_key,
        "Content-Type": "application/json",
    }
    ...
    if r.status_code == 200:
        return True
    print(f"[-] Failed to send abuse.ch results: HTTP {r.status_code} - {r.text}")
    return False
